...
from transformers import LukeConfig, LukeForEntitySpanClassification, LukeTokenizer, RobertaTokenizer
...
def convert_luke_checkpoint(checkpoint_path, metadata_path, entity_vocab_path, pytorch_dump_folder_path, model_size):
    ...
    model = LukeForEntitySpanClassification(config=config).eval()
    ...
    # Check outputs
    tokenizer = LukeTokenizer.from_pretrained(pytorch_dump_folder_path, task="entity_span_classification")

    text = "Beyoncé lives in New York"
    word_start_positions = [0, 8, 14, 17, 21]
    word_end_positions = [7, 13, 16, 20, 25]
    entity_spans = []
    ...
    # https://colab.research.google.com/drive/1U2s0vysQjqJG4zmUm4zpEN4EPvI4Xc9U?usp=sharing
    assert outputs.logits.shape == torch.Size((1, 15, 5))
    assert torch.allclose(outputs.logits[0, 0, :3], torch.tensor([-1.8444, -2.3899, 4.8322]), atol=1e-4)
